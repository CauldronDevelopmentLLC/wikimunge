...
lua_dir = pkg_resources.resource_filename('wikimunge', 'lua/')
if not lua_dir.endswith('/'): lua_dir += '/'

# Substitutions to perform on Lua code from the dump to convert from
# Lua 5.1 to current 5.3
loader_replace_patterns = [(re.compile(src), dst) for src, dst in [
  [r'\\\\\?', r'%\\092?'],
  [r'\\\\\*', r'%\\092*'],
  [r'\\\\\-', r'%\\092-'],
  [r'\\\\\+', r'%\\092+'],
...
def lua_loader(ctx, modname):
  ...
  local_module_ns_name = ctx.name_data.get_name('Module')

  # Local name usually not used in Lua code
  if modname.startswith(('Module:', local_module_ns_name + ':')):
    # Canonicalize the name
    modname = ctx.name_data.canonicalize_template_name(modname)
    ...
    if modname.startswith(('Module:_', local_module_ns_name + ':_')):
      ...
      if data is None:
        module_name_len = len('Module') if modname.startswith('Module') else \
          len(local_module_ns_name)
        title = local_module_ns_name + ':' + modname[module_name_len + 1:]
    ...
    for prefix, exceptions in builtin_lua_search_paths:
      if modname in exceptions: continue
  ...
  if data is None:
    ctx.debug('Module %r not found' % modname)
    return None # We did not find the module
...
def mw_text_decode(text, decodeNamedEntities):
  ...
  if decodeNamedEntities: return html.unescape(text)
  ...
  for m in re.finditer(r'&(lt|gt|amp|quot|nbsp);', text):
    if pos < m.start(): parts.append(text[pos:m.start()])
    pos = m.end()
    tag = m.group(1)

    if   tag == 'lt':   parts.append('<')
    elif tag == 'gt':   parts.append('>')
    elif tag == 'amp':  parts.append('&')
    elif tag == 'quot': parts.append('"')
    elif tag == 'nbsp': parts.append('\xa0')
    else: assert False
...
def mw_text_jsondecode(ctx, s, *rest):
  ...
  def recur(x):
    if isinstance(x, (list, tuple)):
      return ctx.lua.table_from(list(map(recur, x)))
    if not isinstance(x, dict): return x

    # It is a dict.
    if (flags & 1) == 1:
      # JSON_PRESERVE_KEYS flag means we don't convert keys.
      return ctx.lua.table_from({k: recur(v) for k, v in x.items()})
...
def mw_text_jsonencode(s, *rest):
  ...
  def recur(x):
    if isinstance(x, (str, int, float, type(None), type(True))): return x
    ...
    if lupa.lua_type(x) == 'table':
      ...
      if not conv_to_dict:
        ...
        # starting from 1
        if not all(isinstance(k, int) for k in x.keys()): conv_to_dict = True
        else:
          keys = list(sorted(x.keys()))
          if not all(keys[i] == i + 1 for i in range(len(keys))):
            conv_to_dict = True
      ...
      if conv_to_dict:
        return {str(k): recur(v) for k, v in x.items()}

      # Convert to list (JSON array)
      return list(map(recur, x.values()))
...
def get_page_info(ctx, title):
  ...
  redirect_to = ctx.page_redirect(title)
...
def fetch_language_names(ctx, include):
  ...
  if include == 'all': ret = ctx.LANGUAGES_BY_CODE
  else: ret = {'en': ctx.LANGUAGES_BY_CODE['en']}
...
def initialize_lua(ctx):
  ...
  lua = lupa.LuaRuntime(
    unpack_returned_tuples = True, register_eval = False,
    attribute_filter = filter_attribute_access)
  ctx.lua = lua
  set_namespace_data = lua.eval('function(v) NAMESPACE_DATA = v end')
  lua_namespace_data = copy.deepcopy(ctx.name_data.data)
...
def call_lua_sandbox(ctx, invoke_args, expander, parent, timeout):
  ...
  if len(invoke_args) < 2:
    ctx.debug('#invoke %s with too few arguments' % invoke_args)
  ...
  def value_with_expand(frame, fexpander, x):
    ...
    return lua.table_from({'expand': lambda obj: frame[fexpander](x)})
  ...
  def make_frame(pframe, title, args):
    ...
    def callParserFunction(frame, *args):
      ...
      if not isinstance(name, str):
        ...
        if isinstance(new_args, str): new_args = {1: new_args}
        else: new_args = dict(new_args)
        name = name['name'] or ''
      ...
      name = str(name)
      for arg in args[1:]:
        if isinstance(arg, (int, float, str)): new_args.append(str(arg))
        else:
          for k, v in sorted(arg.items(), key = lambda x: str(x[0])):
            new_args.append(str(v))

      name = ctx.name_data.canonicalize_parserfn_name(name)
      if name not in PARSER_FUNCTIONS:
        ctx.debug('lua frame callParserFunction() undefined function %s' % name)
        return ''

      return call_parser_function(ctx, name, new_args, lambda x: x)


    def expand_all_templates(encoded, frame):
      ...
      ctx.expand_stack.append(frame)
      ret = ctx.expand(encoded, parent)
      ctx.expand_stack.pop()
      return ret
    ...
    def preprocess(frame, *args):
      ...
      v = args[0]
      if not isinstance(v, str): v = str(v['text'] or '')

      # Expand all templates, in case the Lua code actually inspects the output.
      ret = expand_all_templates(ctx.encode(v), 'frame:preprocess()')
    ...
    def expandTemplate(frame, *args):
      ...
      encoded = ctx.save_cookie('T', new_args, False)
      ret = expand_all_templates(encoded, 'frame:expandTemplate()')
  ...
  if parent is not None:
    ...
    for k, v in page_args.items():
      if isinstance(k, str): expanded_key_args[expander(k)] = v
      else: expanded_key_args[k] = v
  ...
  try:
    ...
    if not isinstance(ret, (list, tuple)): ok, text = ret, ''
    elif len(ret) == 1: ok, text = ret[0], ''
    else: ok, text = ret[0], ret[1]

  except UnicodeDecodeError:
    ctx.debug('invalid unicode returned from lua by %s: parent %s' % (
      invoke_args, parent))
    ok, text = True, ''
    ...
    while stack_len < len(ctx.expand_stack):
      ctx.expand_stack.pop()

  ctx.lua_depth -= 1
  if ok: return unicodedata.normalize('NFC', '' if text is None else str(text))
  ...
  if isinstance(text, Exception):
    ...
    lst = traceback.format_exception(type(text), value = text,
                                     tb = text.__traceback__)
    for x in lst: parts.append('\t' + x.strip())
    text = '\n'.join(parts)
  ...
  if text.find('\'debug.error\'') >= 0:
    ...
    if 'check deprecated lang param usage' in ctx.expand_stack:
      ctx.debug('LUA error but likely not bug -- in #invoke {} parent {}'
                .format(invoke_args, parent), trace = trace)

    else: ctx.error('LUA error in #invoke %s parent %s' % (invoke_args, parent),
                    trace = trace)

  msg = 'Lua execution error'
  if 'Lua timeout error' in text: msg = 'Lua timeout error'
