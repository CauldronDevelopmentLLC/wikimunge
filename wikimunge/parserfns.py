...
import urllib.parse
import dateparser
from .common import nowiki_quote, MAGIC_NOWIKI_CHAR, ALLOWED_HTML_TAGS
...
def errtag(msg): return '<strong class="error">%s</strong>' % msg
...
def if_fn(ctx, fn_name, args, expander):
  ...
  return expander(arg1 if v else arg2).strip()
...
def iferror_fn(ctx, fn_name, args, expander):
  ...
  if re.search(r'<[^>]*?\sclass="error"', arg0):
    return '' if arg1 is None else expander(arg1).strip()

  return arg0 if arg2 is None else expander(arg2).strip()
...
def ifexpr_fn(ctx, fn_name, args, expander):
  ...
  try: ret = int(cond)
  except ValueError: ret = 0

  return expander(arg1 if ret else arg2).strip()
...
def ifexist_fn(ctx, fn_name, args, expander):
  ...
  return expander(arg1 if exists else arg2).strip()
...
def switch_fn(ctx, fn_name, args, expander):
  ...
  for i in range(1, len(args)):
    ...
    if not m:
      ...
      if last == val: match_next = True
      continue

    k, v = m.groups()
    k = expander(k).strip()
    if k == val or match_next: return expander(v).strip()
    if k == '#default': defval = v
    last = None

  if defval is not None: return expander(defval).strip()
  return last or ''
...
def lst_fn(ctx, fn_name, args, expander):
  ...
  if text is None:
    ctx.warning(
        '{} trying to transclude chapter {!r} from non-existent page {!r}'
  ...
  for m in re.finditer(
          ...
          .format(re.escape(chapter), re.escape(chapter)), text):
...
def tag_fn(ctx, fn_name, args, expander):
  ...
  if tag not in ALLOWED_HTML_TAGS and tag != 'nowiki':
    ctx.warning('#tag creating non-allowed tag <{}> - omitted'.format(tag))
    return '{{' + fn_name + ':' + '|'.join(args) + '}}'

  content = expander(args[1]) if 2 <= len(args) else ''
  attrs = []
  ...
  if len(args) > 2:
    for x in args[2:]:
      ...
      if not m:
        ctx.warning('invalid attribute format {!r} missing name'.format(x))
        continue
  ...
  if not content: ret = '<{}{} />'.format(tag, attrs)
  else: ret = '<{}{}>{}</{}>'.format(tag, attrs, content, tag)

  if tag == 'nowiki':
    ret = MAGIC_NOWIKI_CHAR if len(args) == 0 else nowiki_quote(content)
...
def pagename_fn(ctx, fn_name, args, expander):
  ...
  return t[ofs + 1:] if 0 <= ofs else t
...
def talkpagename_fn(ctx, fn_name, args, expander):
  ...
  if ofs < 0: return ctx.name_data.get_name('Talk') + ':' + ctx.title

  prefix = ctx.title[:ofs]

  if prefix not in ctx.name_data.data:
    return ctx.name_data.get_name('Talk') + ':' + ctx.title

  return ctx.name_data.get_name(prefix + ' talk') + ':' + ctx.title[ofs + 1:]
...
def namespace_fn(ctx, fn_name, args, expander):
  ...
  if ofs >= 0:
    ...
    if ns == 'Project': return ctx.name_data.get_name('Project')
    return ns
...
def subjectspace_fn(ctx, fn_name, args, expander):
  '''Minimal implementation of the SUBJECTSPACE magic word/parser function.'''
  t = expander(args[0]) if args else ctx.title

  ns = ctx.name_data.get(t)
  return ns.name if ns is not None else ''


def talkspace_fn(ctx, fn_name, args, expander):
  '''Minimal implementation of the TALKSPACE magic word/parser function.'''
  t = expander(args[0]) if args else ctx.title

  ns = ctx.name_data.get(t)
  if ns is not None: return ctx.name_data.get_name(ns.name + ' talk')

  return ctx.name_data.get_name('Talk')
...
def dateformat_fn(ctx, fn_name, args, expander):
  ...
  if fmt == 'mdy':
    ...
    if date_only:
      if dt.year == 3333: return dt.strftime('%d %b')
  ...
  if date_only: return dt.date().isoformat()
...
def localurl_fn(ctx, fn_name, args, expander):
  ...
  if arg1:
    return '/w/index.php?title=%s&%s' % (urllib.parse.quote_plus(arg0), arg1)

  return '/wiki/%s' % wikiurlencode(arg0)
...
def fullurl_fn(ctx, fn_name, args, expander):
  ...
  if len(args) > 1:
    for arg in args[1:]:
      ...
      if not m: url += '&' + urllib.parse.quote_plus(arg)
      else: url += ('&' + urllib.parse.quote_plus(m.group(1)) + '=' +
...
def urlencode_fn(ctx, fn_name, args, expander):
  ...
  if fmt == 'PATH':  return urllib.parse.quote(url, safe = '')
  if fmt == 'QUERY': return urllib.parse.quote_plus(url)

  return wikiurlencode(url) # All else in WIKI encoding


def wikiurlencode(url):
  assert isinstance(url, str)
  return urllib.parse.quote(re.sub(r'\s+', '_', url), safe = '/:')
...
def ns_fn(ctx, fn_name, args, expander):
  ...
  name = expander(args[0]).strip() if args else ''
  ns = ctx.name_data.get(name)
...
def titleparts_fn(ctx, fn_name, args, expander):
  ...
  if first < 0: first = max(0, num_parts + first)
  elif first > num_parts: first = num_parts

  if num_return == 0: num_return = num_parts
  elif num_return < 0: num_return = max(0, num_parts + num_return)

  return ''.join(parts[2 * first: 2 * (first + num_return) - 1])
...
def expr_fn(ctx, fn_name, args, expander):
  ...
  tokens = [
    m.group(0) for m in
    re.finditer(r'\d+(\.\d*)?|\.\d+|[a-z]+|!=|<>|>=|<=|[^\s]', full_expr)]
  tokidx = 0

  def expr_error(tok):
    if tok is None: tok = '&lt;end&gt;'
    return errtag('Expression error near %s' % tok)

  def get_token():
    nonlocal tokidx
    if tokidx >= len(tokens): return None
    tok = tokens[tokidx]
    tokidx += 1
    return tok
  ...
  def parse_atom(tok):
    if tok is None: return expr_error(tok)
  ...
  def generic_binary(tok, parser, fns, assoc = 'left'):
    ...
    while True:
      ...
      if isinstance(ret2, str): return ret2
      ret = fn(ret, ret2)
  ...
  def parse_binary_e(tok): return generic_binary(tok, parse_unary, binary_e_fns)

  def parse_unary_fn(tok):
    fn = unary_fns.get(tok)

    if fn is None: return parse_binary_e(tok)
    tok = get_token()
  ...
  def parse_expr(tok): return parse_binary_or(tok)

  tok = get_token()
  ret = parse_expr(tok)

  if isinstance(ret, str): return ret
  if isinstance(ret, float) and ret == math.floor(ret): return str(int(ret))
  return str(ret)
...
def padleft_fn(ctx, fn_name, args, expander):
  ...
  if len(v) < cnt: v = pad[:cnt - len(v)] + v
...
def padright_fn(ctx, fn_name, args, expander):
  ...
  if cnt - len(v) > len(pad): pad = (pad * ((cnt - len(v)) // len(pad)))
  if len(v) < cnt: v = v + pad[:cnt - len(v)]
...
def plural_fn(ctx, fn_name, args, expander):
  ...
  if v == 1: return expander(args[1]).strip() if len(args) >= 2 else ''
...
def month_num_days(ctx, t):
  ...
  if t.month == 2:
    if t.year % 4 == 0 and (t.year % 100 != 0 or t.year % 400 == 0): v = 29

  return v


def time_L(ctx, t):
  if t.year % 4 == 0 and (t.year % 100 != 0 or t.year % 400 == 0): return 1
  return 0


def time_z(ctx, t):
  dt = datetime.datetime(year = t.year, month = 1, day = 1, tzinfo = t.tzinfo)
  return (t - dt).days
...
def time_fn(ctx, fn_name, args, expander):
  ...
  if dt.startswith('@'):
    try: t = datetime.datetime.fromtimestamp(float(dt[1:]))
    except ValueError:
      ctx.warning('bad time syntax in {}: {!r}'.format(fn_name, orig_dt))
      return errtag('Bad time syntax: %s' % html.escape(orig_dt))

  else:
    t = dateparser.parse(dt, settings = settings)

    if t is None:
      ctx.warning('unrecognized time syntax in %s: %s' % (fn_name, orig_dt))
      return errtag('Bad time syntax: %s' % html.escape(orig_dt))

  def fmt_repl(m):
    f = m.group(0)

    if len(f) > 1 and f.startswith('"') and f.endswith('"'): return f[1:-1]
...
def pos_fn(ctx, fn_name, args, expander):
  ...
  if not offset or not offset.isdigit(): offset = '0'
  offset = int(offset)
...
def sub_fn(ctx, fn_name, args, expander):
  ...
  if start < 0: start = max(0, len(arg0) + start)
  start = min(start, len(arg0))

  try: length = int(length)
  except ValueError: length = 0

  if length == 0: length = max(0, len(arg0) - start)
  elif length < 0: length = max(0, len(arg0) - start + length)
...
def pad_fn(ctx, fn_name, args, expander):
  ...
  if not cnt.isdigit():
    ctx.warning('pad length is not integer: %s' % cnt)
    cnt = 0

  else: cnt = int(cnt)

  if cnt - len(v) > len(pad): pad = (pad * ((cnt - len(v)) // len(pad) + 1))

  if len(v) < cnt:
    padlen = cnt - len(v)

    if direction == 'right': v = v + pad[:padlen]
    elif direction == 'center':
        v = pad[:padlen // 2] + v + pad[:padlen - padlen // 2]
    else: v = pad[:padlen] + v # left
...
def explode_fn(ctx, fn_name, args, expander):
  ...
  if position < 0: position = len(parts) + position
  if position < 0 or position >= len(parts): return ''
...
def unimplemented_fn(ctx, fn_name, args, expander):
  ctx.error('unimplemented parserfn %s' % fn_name)
  return '{{' + fn_name + ':' + '|'.join([str(arg) for arg in  args]) + '}}'
...
def call_parser_function(ctx, fn_name, args, expander):
  ...
  if fn_name not in PARSER_FUNCTIONS:
    ctx.error('unrecognized parser function %s' % fn_name)
    return ''
  ...
  if have_keyed_args and not accept_keyed_args:
    ctx.error('parser function %s does not support named arguments: %s' % (
      fn_name, args))
    return ''
