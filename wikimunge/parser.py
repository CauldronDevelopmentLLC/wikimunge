...
from .expander  import finalize_expand, preprocess_text
from .common    import ALLOWED_HTML_TAGS, MAGIC_NOWIKI_CHAR, MAGIC_FIRST, \
  MAGIC_LAST, nowiki_quote


# Set of tags that can be parents of 'flow' parents
HTML_FLOW_PARENTS = set(
  k for k, v in ALLOWED_HTML_TAGS.items()
...
def _parser_merge_str_children(ctx):
  ...
  for x in node.children:
    if isinstance(x, str): strings.append(x)
      ...
      if strings:
        s = finalize_expand(ctx, ''.join(strings))
...
def _parser_pop(ctx, warn_unclosed):
  ...
  if warn_unclosed and node.kind in MUST_CLOSE_KINDS:
    if node.kind == NodeKind.HTML:
      ...
      # in Wiktionary that let's suppress any warnings about them.
      pass
  ...
  # If the node has arguments, move remamining children to be the last argument
...
def close_begline_lists(ctx):
  ...
  if not ctx.beginning_of_line: return
...
def text_fn(ctx, token):
  ...
  if ctx.beginning_of_line:
    ...
    if token.startswith(' ') or token.startswith('\t'):
      if ctx.parser_stack[-1].kind in (NodeKind.TABLE, NodeKind.TABLE_ROW):
        return
  ...
  if (node.children and isinstance(node.children[-1], WikiNode) and
      ...
      not node.children[-1].children and not ctx.suppress_special):
...
def hline_fn(ctx, token):
  ...
  while True:
    node = ctx.parser_stack[-1]
    if node.kind in (
        NodeKind.ROOT, NodeKind.LEVEL2, NodeKind.TABLE, NodeKind.TABLE_CAPTION,
        NodeKind.TABLE_ROW, NodeKind.TABLE_HEADER_CELL, NodeKind.TABLE_CELL,
        NodeKind.HTML): break
...
def subtitle_start_fn(ctx, token):
  ...
  if ctx.pre_parse: return text_fn(ctx, token)
  ...
  while any(x.kind in kind_to_level for x in ctx.parser_stack):
    node = ctx.parser_stack[-1]
    if kind_to_level.get(node.kind, 99) < level: break
    if node.kind == NodeKind.HTML and node.args != 'span':
      break
    _parser_pop(ctx, True)
...
def subtitle_end_fn(ctx, token):
  ...
  while True:
    node = ctx.parser_stack[-1]
    if node.kind in kind_to_level: break
    _parser_pop(ctx, True)
...
def italic_fn(ctx, token):
  ...
  if not _parser_have(ctx, NodeKind.ITALIC) or node.kind == NodeKind.LINK:
  ...
  while True:
    ...
    if node.kind == NodeKind.BOLD: push_bold = True
    _parser_pop(ctx, False)

  if push_bold: _parser_push(ctx, NodeKind.BOLD)
...
def bold_fn(ctx, token):
  ...
  if not _parser_have(ctx, NodeKind.BOLD) or node.kind == NodeKind.LINK:
    # Push new formatting node
  ...
  while True:
    ...
    if node.kind == NodeKind.ITALIC: push_italic = True
    _parser_pop(ctx, False)

  if push_italic: _parser_push(ctx, NodeKind.ITALIC)
...
def elink_end_fn(ctx, token):
  ...
  if not _parser_have(ctx, NodeKind.URL): return text_fn(ctx, token)
  ...
  while True:
    ...
    if node.kind in (NodeKind.TEMPLATE, NodeKind.TEMPLATE_ARG, NodeKind.LINK,
...
def url_fn(ctx, token):
  ...
  node = ctx.parser_stack[-1]
  if node.kind == NodeKind.URL: return text_fn(ctx, token)

  node = _parser_push(ctx, NodeKind.URL)
  text_fn(ctx, token)
  _parser_pop(ctx, False)

  if suffix: text_fn(ctx, suffix)
...
def magic_fn(ctx, token):
  ...
  if not ctx.has_cookie(idx): return text_fn(ctx, token)

  kind, args, nowiki = ctx.load_cookie(idx)
  ctx.beginning_of_line = False
  if kind == 'T':
    if nowiki:
      process_text(ctx, '&lbrace;&lbrace;' + '&vert;'.join(args) +
                   '&rbrace;&rbrace;')
      return
    ...
    while True:
      node = ctx.parser_stack[-1]
      if node.kind == NodeKind.ROOT: break
    ...
    if nowiki:
      process_text(ctx, '&lbrace;&lbrace;&lbrace;' + '&vert;'.join(args) +
    ...
    if nowiki:
      process_text(ctx, '&lsqb;&lsqb;' + '&vert;'.join(args) + '&rsqb;&rsqb;')
      return
    ...
    if not nowiki and args and (
      ...
      # a URL.
      if not _parser_have(ctx, NodeKind.URL): text_fn(ctx, ']')
      else:
    ...
    else: process_text(ctx, '[' + '&vert;'.join(args) + ']')
  ...
  else: ctx.error('magic_fn: unsupported cookie kind %s' % kind)
...
def table_check_attrs(ctx):
  ...
  node = ctx.parser_stack[-1]
  if node.kind != NodeKind.TABLE: return
  _parser_merge_str_children(ctx)

  if len(node.children) != 1 or not isinstance(node.children[0], str): return
...
def table_row_check_attrs(ctx):
  ...
  close_begline_lists(ctx)
  node = ctx.parser_stack[-1]
  if node.kind != NodeKind.TABLE_ROW: return
  _parser_merge_str_children(ctx)
  if len(node.children) != 1 or not isinstance(node.children[0], str): return
  attrs = node.children.pop()
  parse_attrs(node, attrs)
...
def table_caption_fn(ctx, token):
  ...
  if not _parser_have(ctx, NodeKind.TABLE): return text_fn(ctx, token)

  while True:
    node = ctx.parser_stack[-1]
    if node.kind == NodeKind.TABLE: break
    _parser_pop(ctx, True)
...
def table_cell_fn(ctx, token):
  ...
  if (token == '|' and not ctx.wsp_beginning_of_line and
      not ctx.beginning_of_line):
    # This might separate attributes for captions, header cells, and data cells
    _parser_merge_str_children(ctx)
    node = ctx.parser_stack[-1]
    if (not node.attrs and len(node.children) == 1 and
      isinstance(node.children[0], str)):
      if node.kind in (NodeKind.TABLE_CAPTION, NodeKind.TABLE_HEADER_CELL,
                       NodeKind.TABLE_CELL):
        attrs = node.children.pop()
        parse_attrs(node, attrs)
        return
  ...
  while True:
    node = ctx.parser_stack[-1]
    if node.kind == NodeKind.TABLE_ROW: break
    if node.kind == NodeKind.TABLE:
      _parser_push(ctx, NodeKind.TABLE_ROW)
      break

    if node.kind == NodeKind.TABLE_CAPTION: return text_fn(ctx, token)
...
def list_fn(ctx, token):
  ...
  if node.kind in (NodeKind.LINK, NodeKind.URL): return text_fn(ctx, token)
  ...
  while True:
    ...
    if node.kind == NodeKind.LIST_ITEM and len(node.args) < len(token):
      for i in range(len(node.args)):
        if token[i] not in (':', node.args[i]): break  # Tokens do not match
    ...
    if node.kind in kind_to_level: break  # Always break before section header

    # There are various kinds of nodes that can contain lists.  We won't
    # pop them.
    if node.kind in (
        NodeKind.HTML, NodeKind.TEMPLATE, NodeKind.TEMPLATE_ARG,
        NodeKind.PARSER_FN, NodeKind.TABLE, NodeKind.TABLE_HEADER_CELL,
        NodeKind.TABLE_ROW, NodeKind.TABLE_CELL): break
...
def parse_attrs(node, attrs):
  ...
  for m in re.finditer(r'(?si)\b([^"\'>/=\0-\037\s]+)'
    ...
    if value.startswith('\'') or value.startswith('"'): value = value[1:-1]
...
def tag_fn(ctx, token):
  ...
  if (token.startswith('<<') or _parser_have(ctx, NodeKind.TEMPLATE) or
  ...
  if m:
    ...
    if name == 'noinclude' and also_end: return

    # Handle <pre> start tag
    if name == 'pre':
      node = _parser_push(ctx, NodeKind.PRE)
      parse_attrs(node, attrs)
      if also_end: _parser_pop(ctx, False)
      else: ctx.pre_parse = True
      return
    ...
    while True:
      ...
      if node.kind != NodeKind.HTML: break
      if node.args in permitted_parents: break

      close_next = ALLOWED_HTML_TAGS.get(node.args, {}).get('close-next', [])
    ...
    if no_end_tag or also_end: _parser_pop(ctx, False)
    return

  # Since it was not a start tag, it should be an end tag
  m = re.match(r'<\s*/\s*([-a-zA-Z0-9]+)\s*>', token)
  if m is None:
    raise RuntimeError('Could not match end tag token: {!r}'.format(token))
  ...
  while True:
    ...
    if node.kind == NodeKind.HTML:
      ...
      close_next = ALLOWED_HTML_TAGS.get(node.args, {}).get('close-next', None)
...
for x in MAGIC_WORDS: tokenops[x] = magicword_fn
  ...
  for line in lines:
    ...
    for i, part in enumerate(parts):
      ...
      for m in re.finditer(token_re, part):
        start = m.start()
        if pos != start: yield False, part[pos:start]
        pos = m.end()
        token = m.group(0)
        ...
        else: yield True, token

      if pos != len(part): yield False, part[pos:]
...
def process_text(ctx, text):
  ...
  for is_token, token in token_iter(ctx, text):
    node = ctx.parser_stack[-1]
    if not is_token: text_fn(ctx, token) # Process it as normal text.
      ...
      elif token.startswith('>=='): token = token[1:]
      text_fn(ctx, token)

    else:
      # Process it as a token.  In some contexts some tokens may still
      # be interpreted as text.
      if token in tokenops: tokenops[token](ctx, token)
      # Note: < added by tokenizer
      elif token.startswith('<=='): subtitle_start_fn(ctx, token)
      # Note: > added by tokenizer
      elif token.startswith('>=='): subtitle_end_fn(ctx, token)
      elif token.startswith('<'): tag_fn(ctx, token) # HTML tag like construct
      elif token.startswith('----'): hline_fn(ctx, token)
      elif re.match(list_prefix_re, token): list_fn(ctx, token)
        ...
        t2 = token.strip()
        if t2 in tokenops: tokenops[t2](ctx, t2)
        else: text_fn(ctx, token)
...
def parse_encoded(ctx, text):
  ...
  characters.  Parses the encoded string and returns the parse tree.'''
  ...
  finally: ctx.parser_stack = None

  return ret


def parse(ctx, text):
  '''Parses the given text into a parse tree (WikiNode tree)'''
  ...
  return parse_encoded(ctx, ctx.encode(text))
